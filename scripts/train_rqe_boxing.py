#!/usr/bin/env python3
"""
Train RQE-MAPPO on Atari Boxing

Uses standalone RQE-MAPPO (practical version) with CNN networks for visual observations.
"""
...
from algorithms.rqe_mappo import RQE_MAPPO, RQEConfig
...
class CNNDistributionalCritic(nn.Module):
    """CNN-based distributional critic (state-value function)"""
    ...
    def forward(self, obs: torch.Tensor) -> torch.Tensor:
        ...
        """
        features = self.cnn(obs)
        logits = self.fc(features)  # [batch, n_atoms]
    ...
    def get_risk_value(
        ...
        """Compute risk-adjusted state values"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train RQE-MAPPO on Boxing"
    )
...
def collect_rollout(env, agents, batch_size, max_cycles=10000):
    ...
    agent_names = ['first_0', 'second_0']  # Fixed agent names for Boxing
    observations = []
    actions = []
    rewards = []
    dones = []
    ...
    while timesteps < batch_size:
        ...
        for step in range(max_cycles):
            # Check if all agents are still active
            if len(env.agents) < n_agents:
                break

            # Preprocess observations
            obs_list = [preprocess_obs(obs[agent]) for agent in agent_names]
            ...
            action_dict = {
                ...
                for i, agent in enumerate(agent_names) if agent in env.agents
            }
            ...
            if len(env.agents) < n_agents:
                # Episode ended mid-step, use zeros for missing agents
                next_obs_list = []
                reward_list = []
                for i, agent in enumerate(agent_names):
                    if agent in next_obs:
                        next_obs_list.append(preprocess_obs(next_obs[agent]))
                        reward_list.append(reward_dict.get(agent, 0.0))
                    else:
                        # Use last observation and zero reward for terminated agents
                        next_obs_list.append(obs_list[i])
                        reward_list.append(0.0)
                done_any = True
            else:
                next_obs_list = [preprocess_obs(next_obs[agent]) for agent in agent_names]
                reward_list = [reward_dict[agent] for agent in agent_names]
...
def main():
    ...
    agents = RQE_MAPPO(actors, critics, config)

    # Experiment name
    exp_name = args.exp_name or f"RQE_MAPPO_Boxing_tau{args.tau}_eps{args.epsilon}"
    ...
    print("=" * 70)
    print(f"Starting RQE-MAPPO Training on Atari Boxing")
    print("=" * 70)
