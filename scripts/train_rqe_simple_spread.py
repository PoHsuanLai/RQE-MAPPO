#!/usr/bin/env python3
"""
Train RQE-PPO on PettingZoo Simple Spread Environment

Simple Spread is a cooperative multi-agent environment where 3 agents must
cover 3 landmarks while avoiding collisions with each other.

Rewards:
- Positive: Landmarks covered by agents
- Negative: Agent-agent collisions
- Risk tradeoff: Agents must coordinate without colliding

This is a good test for risk-sensitive coordination in continuous space.
"""
...
from algorithms.rqe_ppo_rllib import RQEPPO, RQEPPOConfig


def parse_args():
    parser = argparse.ArgumentParser(
        description="Train RQE-PPO on Simple Spread environment"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Experiment name (default: RQE_PPO_SimpleSpread_tau{tau}_eps{epsilon})"
    )
...
def main():
    ...
    # Experiment name
    exp_name = args.exp_name or f"RQE_PPO_SimpleSpread_tau{args.tau}_eps{args.epsilon}"

    # Run training
    print("="*70)
    print(f"Starting RQE-PPO Training on Simple Spread")
    print("="*70)
